...
_"Redux là state management library phổ biến nhất. Tại sao bạn chọn Zustand? Redux có gì hơn?"_
...
_"ProductCard dùng ở HomePage, ProductsPage, và ProductDetailPage (similar products).
Sao không tạo 3 component khác nhau? Làm sao handle tất cả case?"_
...
const ProductCard = ({
  ...
  return (
    <div className={`card ${className}`}>
      ...
      {showAddToCart ? <button onClick={handleAdd}>Thêm vào giỏ</button> : null}
    </div>
  );
};
```
...
_"Tôi add sản phẩm vào giỏ. Sau đó shutdown browser. Khi mở lại, giỏ vẫn còn.
Dữ liệu được lưu ở database Backend hay localStorage?"_
...
_"User click 'Thanh toán VNPay'. Sau đó gì xảy ra? Dữ liệu flow như nào?
Backend + Frontend có làm gì?"_
...
_"User gõ 'Ghế gỗ dưới 2 triệu'. Sao backend biết đó là category=Chair, material=Wood, maxPrice=2M?
Có dùng NLP/ML không? Hay regex pattern matching?"_
...
const extractConstraints = (userInput) => {
  ...
  if (
    Object.keys(materialKeywords).some((pattern) =>
  ...
  const pricePattern =
    /từ\s*([\d.kmتriệu]+)\s*đến\s*([\d.kmتriệu]+)|dưới\s*([\d.kmتriệu]+)|từ\s*([\d.kmتriệu]+)\s*trở lên/i;
...
_"Trước dây bạn báo card bị lệch khi có sản phẩm 'kệ'. Sau đó fix.
Vấn đề thực sự là gì? Image size sao?"_
...
_"ProductCard hiển thị 'Đã bán 5'. Số này từ đâu? Backend tính như nào?"_
...
_"User đang loggedin, thì token hết hạn. Khi call API sao?
User bị logout hay refresh token tự động?"_
...
_"Build xong, folder `dist/` có những gì? Gửi lên server như nào?
Backend cần config gì không?"_
...
_"Ngày nay TypeScript là standard. Tại sao project dùng plain JavaScript?
Nếu dùng TypeScript, cần thêm bao lâu?"_
...
_"Nếu phải thêm 10 tính năng mới, 5 engineer, 6 tháng tới.
Architecture hiện tại có scale được không? Cần refactor gì?"_
